...
- Tipografia: `Sora` nos títulos (`--font-display`) e `Inter` no texto (`--font-sans`), self-hosted via `fontsource`
...
- A folha de impressão (`.folha-impressao`) é tema único: sempre `#fff`/`#000`, e nunca contém emoji
- Emojis contextuais fazem parte da linguagem visual (sidebar, títulos, passos, empty states, toasts), sempre em `<span aria-hidden="true">` e nunca acumulados com um ícone no mesmo rótulo — ver [DESIGN.md](docs/blueprint/DESIGN.md) §7
...
enquanto não existirem, valem apenas as pastas e o padrão de nome abaixo.

Cada feature tem um **ID sequencial de 2 dígitos com zero à esquerda** (`01`, `02`, …),
atribuído na constituição pela ordem em que aparece na seção "Features do projeto". Esse ID
prefixa o arquivo da feature em **todas as etapas**, no formato `NN-<slug>`:
`docs/specs/02-checkout.md`, `docs/plans/02-checkout.md` e `docs/tasks/02-checkout.md`
referem-se todos à feature `checkout` (ID `02`, slug `checkout`). O ID é estável — uma vez
atribuído não muda, e IDs de features removidas não são reaproveitados (gaps são permitidos).
Features novas recebem o próximo ID livre (maior ID atual + 1).
